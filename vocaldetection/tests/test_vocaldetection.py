...
import vocaldetection as vd

data_path = op.join(vd.__path__[0], 'data')
...
def test_transform_data():
    ...
    x1, y1, n1 = vd.transform_data(ortho)
    x2, y2, n2 = vd.transform_data(op.join(data_path, 'ortho.csv'))
    ...
    my_x, my_y, my_n = vd.transform_data(my_data)
...
def test_opt_err_func():
    ...
    my_y = my_x
    my_err = vd.opt_err_func(my_params, my_x, my_y, my_silly_func)
    ...
    my_err = vd.opt_err_func(my_params, my_x, my_y, not_so_silly_func)
    # Since x and y are equal, the error is zero:
    npt.assert_equal(my_err, np.zeros(my_x.shape[0]))
...
def test_Model():
    ...
    target_mu = 0.5
    target_sigma = 1
    target_y = vd.cumgauss(x, target_mu, target_sigma)
...
def test_params_regression():
    ...
    model = vd.Model()
    ortho_x, ortho_y, ortho_n = vd.transform_data(op.join(data_path,
                                                          'ortho.csv'))

    para_x, para_y, para_n = vd.transform_data(op.join(data_path,
                                                       'para.csv'))
...
def test_hello_function():
    """ This is my first function test to realize what I should do """

    npt.assert_equal(vd.hello_function(3), 3**3)
    npt.assert_equal(vd.hello_function(4), 4**4)
