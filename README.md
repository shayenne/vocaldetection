## vocaldetection
[![Build Status](https://travis-ci.org/uwescience/vocaldetection.svg?branch=master)](https://travis-ci.org/uwescience/vocaldetection)
...
instructions at the [bottom of this page](#using-vocaldetection-as-a-template).
...
`vocaldetection/vocaldetection.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `vocaldetection.py` in directory called
`vocaldetection`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import vocaldetection as sb` in an
interactive Python session, the classes and functions defined inside of the
`vocaldetection.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the vocaldetection module, the main classes defined at the bottom of
...
Either way, you can create a `vocaldetection/data` folder in which you can
...
(typically, something like our `vocaldetection/tests/test_vocaldetection.py`). Within each
...
have one such test in `vocaldetection/tests/test_vocaldetection.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/vocaldetection`),
and type:

    py.test vocaldetection

This will exercise all of the tests in your code directory. If a test fails, you
will see a message such as:


    vocaldetection/tests/test_vocaldetection.py .F...
    ...
    vocaldetection/tests/test_vocaldetection.py:49: AssertionError
...
To document `vocaldetection` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/vocaldetection/blob/master/doc/_static/RTD-advanced-conf.png)

 http://vocaldetection.readthedocs.org/en/latest/
...
A `vocaldetection/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/vocaldetection) for the software. This
...
`vocaldetection` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `vocaldetection`, we use the
...
[here](https://github.com/uwescience/vocaldetection/blob/master/scripts/Figure1.ipynb).
...
### Using `vocaldetection` as a template
...
Under "old repository clone URL" enter: https://github.com/uwescience/vocaldetection
...
`vocaldetection/vocaldetection.py` to be called `smallish/smallish.py`

	git mv vocaldetection smallish
	git mv smallish/vocaldetection.py smallish/smallish.py
	git mv smallish/tests/test_vocaldetection.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `vocaldetection` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `vocaldetection` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `vocaldetection` in them. Type
the following to see where all these files are:

	git grep vocaldetection

You can replace `vocaldetection` for `smallish` quickly with:

	git grep -l 'vocaldetection' | xargs sed -i 's/vocaldetection/smallish/g'
...
Edit all the mentions of `vocaldetection` in the `vocaldetection/__init__.py` file, and in
the `vocaldetection/version.py` file as well.
